...
class NormalizingFlow(MLModel):
    ...
    def loss_fn(
        ...
        return -self.flow.log_prob(x, conditional=conditional).mean()
    ...
    def fit(
        ...
        # Shuffle data and conditions together
        indices = torch.randperm(x_prime.shape[0], device=self.device)
        x_prime = x_prime[indices, ...]
        if conditional is not None:
            conditional = conditional[indices, ...]

        if n_samples is not None:
            x_prime = x_prime[:n_samples]
            if conditional is not None:
                conditional = conditional[:n_samples]
        ...
        if conditional is not None:
            if torch.isnan(conditional).any():
                raise ValueError("Conditional data contains NaN values.")
            if not torch.isfinite(conditional).all():
                raise ValueError("Conditional data contains infinite values.")
        # Split into training and validation sets
        n = x_prime.shape[0]
        split = -int(validation_fraction * n)
        x_train = torch.as_tensor(
            x_prime[:split],
            dtype=torch.get_default_dtype(),
            device=self.device,
        )
        conditional_train = conditional[:split] if conditional is not None else None
        logger.info(
        ...
        conditional_val = conditional[split:] if conditional is not None else None

        train_data = (
            torch.utils.data.TensorDataset(x_train, conditional_train)
            if conditional_train is not None
            else torch.utils.data.TensorDataset(x_train)
        )
        val_data = (
            torch.utils.data.TensorDataset(x_val, conditional_val)
            if conditional_val is not None
            else torch.utils.data.TensorDataset(x_val)
        )
        ...
        for _ in tqdm.tqdm(range(n_epochs)):
            ...
            for batch in dataset:
                x_batch = batch[0]
                conditional_batch = batch[1] if len(batch) == 2 else None
                loss = self.loss_fn(x_batch, conditional_batch)
                optimizer.zero_grad()
                loss.backward()
    ...
    def log_prob(
        ...
        with torch.no_grad():
            ...
            conditional_in_bounds = (
                conditional[in_bounds]
                if conditional is not None else None
            )
            log_prob[in_bounds] = self.flow.log_prob(
                x_prime,
                conditional=conditional_in_bounds,
    ...
    def prob(self, x, conditional=None):
        return np.exp(self.log_prob(x, conditional=conditional))
    ...
    def to_hdf5(self, h5py_file: h5py.File, group_name: str = "model") -> None:
        ...
        g.attrs["conditions"] = self.conditions
...
class MLStatistic:
    ...
    def prob(self, x, conditional=None):
        return self.model.prob(x, conditional=conditional)
