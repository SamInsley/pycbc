...
from .eventmgr_cython import logsignalrateinternals_computepsignalbinsNF
...
class PhaseTDTHANFStatistic(QuadratureSumStatistic):
    """
    Statistic that re-weights combined newsnr using coinc parameters.
    ...
    def __init__(self, sngl_ranking, files=None, ifos=None,
        ...
        self.pdif = {}
        self.sdif = {}
        print("INIT:", id(self), type(self.pdif))
        for comp in range(1, self.num_comps + 1):
            self.pdif[comp] = numpy.zeros(128, dtype=numpy.float64)
            self.sdif[comp] = numpy.zeros(128, dtype=numpy.float64)
    ...
    def get_hist(self, ifos=None, num_comps=None):
        ...
        self.model_relfac = self.model.metadata.get("sensitivity_ratios")
    ...
    def logsignalrate(self, stats, shift, to_shift):
        ...
        for comp in range(1, self.num_comps + 1):
            ps[comp] = {
                ifo: numpy.array(
                    stats[ifo][f'coa_phase_comp_{comp}'],
                    dtype=numpy.float32,
                    ndmin=1
                )
                for ifo in self.ifos
        ...
        ref_ifo = self.model_ifos[0]
        tref = ts[ref_ifo]
        sigref = sigs[ref_ifo]
        pref = {comp: ps[comp][ref_ifo]
                for comp in range(1, self.num_comps + 1)}

        sref = {comp: ss[comp][ref_ifo]
                for comp in range(1, self.num_comps + 1)}
        ...
        other_ifos = [ifo for ifo in self.ifos if ifo != ref_ifo]
        for ifo in other_ifos:
            pifo = {comp: ps[comp][ifo]
                    for comp in range(1, self.num_comps + 1)}

            sifo = {comp: ss[comp][ifo]
                    for comp in range(1, self.num_comps + 1)}
            length = len(tref)
            while length > len(self.tdif):
                newlen = len(self.tdif) * 2
                self.tdif = numpy.zeros(newlen, dtype=numpy.float64)
            for comp in range(1, self.num_comps + 1):
                # Assign cached memory
                length = len(pref[comp])
                while length > len(self.pdif[comp]):
                    newlen = len(self.pdif[comp]) * 2
                    self.pdif[comp] = numpy.zeros(newlen, dtype=numpy.float64)
                    self.sdif[comp] = numpy.zeros(newlen, dtype=numpy.float64)

            # Calculate differences
            logsignalrateinternals_computepsignalbinsNF(
                self.num_comps,
                self.pdif,
                self.tdif,
                self.sdif,
            ...
            binned.append(self.tdif[:length].copy())

            for comp in range(1, self.num_comps + 1):
                binned.append(self.pdif[comp][:length].copy())

            for comp in range(1, self.num_comps + 1):
                binned.append(self.sdif[comp][:length].copy())
        ...
        block_size = 1 + 2 * self.num_comps

        sdif_sum = numpy.zeros(len(x))

        for start in range(0, x.shape[1], block_size):
            sdif_start = start + 1 + self.num_comps
            sdif_end = start + block_size

            sdif_sum += x[:, sdif_start:sdif_end].sum(axis=1)
...
# FIXME: This class (and probably the one above) need properly integrating into
#        the new statistic system. Much duplication present here.
class ExpFitFgBgNormTHANFStatistic(PhaseTDTHANFStatistic,
                                 ExpFitStatistic):
    """
    Statistic combining PhaseTD, ExpFitBg and additional foreground info.
    """
    ...
    def __init__(self, sngl_ranking, files=None, ifos=None,
        ...
        # PhaseTD statistic single_dtype plus network sensitivity benchmark
        PhaseTDTHANFStatistic.__init__(self, sngl_ranking, files=files,
                                     ifos=self.ifos, num_comps=num_comps, **kwargs)
        self.single_dtype.append(('benchmark_logvol', numpy.float32))
    ...
    def coinc_lim_for_thresh(self, s, thresh, limifo,
        ...
        # Safety against subclassing and not rethinking this
        allowed_names = ['ExpFitFgBgNormTHANFStatistic',
                         'ExpFitFgBgNormTHAStatistic',
                         'ExpFitFgBgNormTHABBHStatistic',
                         'DQExpFitFgBgNormTHAStatistic']
        self._check_coinc_lim_subclass(allowed_names)
...
statistic_dict = {
    ...
    "phasetd_exp_fit_fgbg_norm_tha_NF": ExpFitFgBgNormTHANFStatistic,
}
