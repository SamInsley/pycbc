...
class EventManager(object):
    ...
    def newsnr_threshold(self, threshold):
        """ Remove events with newsnr (or marg_newsnr, if a
        marginalization grid file was supplied for this run) smaller
        ...
        if getattr(self.opt, 'marginalized_grid_file', None):
            # A marginalization grid was supplied for this run -- rank
            # by marg_newsnr (marg_lnl converted to an SNR-equivalent
            # via sqrt(2*marg_lnl), then the standard chi-squared
            # reweighting -- see pycbc.events.ranking.marg_newsnr)
            # instead of plain SNR. Events with marg_lnl = NaN (no grid
            # coverage for that template) pass through this threshold
            # unaffected: NaN < threshold is False in numpy, so those
            # events are kept rather than incorrectly discarded --
            # matching --marg-lnl-threshold's own convention in
            # pycbc_inspiral_tha.
            nsnrs = ranking.marg_newsnr(
                self.events['marg_lnl'],
