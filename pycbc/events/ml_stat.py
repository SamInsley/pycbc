...
class NormalizingFlow(MLModel):
    ...
    def sample(self, n_samples: int = 1, conditional: Optional[np.ndarray] = None) -> np.ndarray:
...
class MLStatistic:
    ...
    def log_prob(self, x: np.ndarray, conditional: Optional[np.ndarray] = None) -> np.ndarray:
        """
